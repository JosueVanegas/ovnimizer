...
export function FileBase64() {
  const t = useTranslations('tools.base64-file')
  ...
  return (
    <div className="space-y-4">
      ...
      {mode === 'encode' ? (
        <>
          <FileDrop onFile={onFile} hint={t('dropFile')} />
          {dataUri && meta && (
            <>
              <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-muted/20 p-3">
                ...
                <div className="min-w-0 text-xs text-muted-foreground">
                  ...
                  <p>{meta.type} · {formatBytes(meta.size)} · {t('asBase64', { size: formatBytes(dataUri.length) })}</p>
                </div>
              </div>
        ...
        <>
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1 space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground">{t('filename')}</label>
