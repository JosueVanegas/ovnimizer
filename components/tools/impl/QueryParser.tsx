...
export function QueryParser() {
  const t = useTranslations('tools.query-parser')
        ...
        <>
          <div className="overflow-hidden rounded-xl border border-border/50">
            <div className="grid grid-cols-[1fr_1.5fr] bg-muted/40 text-xs font-semibold text-muted-foreground">
              <span className="px-3 py-2">{t('key')}</span>
              <span className="border-l border-border/50 px-3 py-2">{t('value')}</span>
            </div>
            {rows.map(([k, v], i) => (
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-muted-foreground">{t('asJson')}</label>
