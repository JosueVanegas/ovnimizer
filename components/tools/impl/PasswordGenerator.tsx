...
function strength(length: number, poolCount: number): { label: string; pct: number; color: string } {
  const poolSize = poolCount * 26
  const bits = length * Math.log2(Math.max(poolSize, 1))
  if (bits < 40) return { label: 'Weak', pct: 33, color: 'bg-destructive' }
  if (bits < 70) return { label: 'Fair', pct: 66, color: 'bg-amber-500' }
  return { label: 'Strong', pct: 100, color: 'bg-ufo-green' }
}
...
export function PasswordGenerator() {
  ...
  const s = strength(length, enabled.length)
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-xs text-muted-foreground">Generated locally with the Web Crypto CSPRNG.</p>
    </div>
  )
}
