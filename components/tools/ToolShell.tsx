...
export async function ToolShell({ tool, children }: { tool: ToolMeta; children: React.ReactNode }) {
  ...
  const title = tt(`${tool.slug}.title`)
  const description = tt(`${tool.slug}.desc`)
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 md:px-8 pt-36 pb-16 space-y-6">
        ...
        {related.length > 0 && (
          <section className="space-y-3 pt-2">
            <h2 className="text-sm font-semibold text-muted-foreground">{tc('relatedTools')}</h2>
