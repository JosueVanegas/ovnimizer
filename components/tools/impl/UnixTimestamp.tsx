  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border/50 bg-muted/20 p-3">
        <span className="text-xs font-semibold text-muted-foreground">{t('currentTime')}</span>
      ...
      {input.trim() && !date && <p className="text-sm text-destructive">{t('unrecognized')}</p>}

      {date && (
        <div className="rounded-xl border border-border/50 bg-muted/10 px-4">
          <Row label={t('unixSeconds')} value={String(Math.floor(date.getTime() / 1000))} />
          <Row label={t('unixMs')} value={String(date.getTime())} />
          <Row label={t('iso')} value={date.toISOString()} />
          <Row label={t('utc')} value={date.toUTCString()} />
          <Row label={t('local')} value={date.toString()} />
        </div>
      )}
    </div>
  )
